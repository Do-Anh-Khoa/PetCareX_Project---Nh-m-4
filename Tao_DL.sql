...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO THU_CUNG VALUES ('TC'+CAST(@i AS VARCHAR)+'_'+CAST(@j AS VARCHAR), N'Pet', @LoaiPet, CASE WHEN (ABS(CHECKSUM(NEWID()))%2)=0 THEN N'Đực' ELSE N'Cái' END, @SK, 'KH'+CAST(@i AS VARCHAR));
        SET @j = @j + 1;
    END
    SET @i = @i + 1;
...
BEGIN
    ...
    DECLARE @TrangThaiHD NVARCHAR(50) = CASE WHEN (ABS(CHECKSUM(NEWID())) % 10) = 0 THEN N'ChuaThanhToan' ELSE N'DaThanhToan' END;

    INSERT INTO HOA_DON (MaHD, NgayLap, TongTien, HinhThucThanhToan, MaNV, MaKH, MaCN, TrangThai)
    VALUES (@MaHD, @DateRun, 0, 'TienMat', @MaNV, @MaKH, @MaCN, @TrangThaiHD);
    ...
    BEGIN
        ...
        DECLARE @NumItem INT = (ABS(CHECKSUM(NEWID())) % 5) + 1; 
        DECLARE @k INT = 1;
        DECLARE @UsedSP TABLE (ID VARCHAR(20)); DELETE FROM @UsedSP;

        WHILE @k <= @NumItem
        BEGIN
            DECLARE @RndSP VARCHAR(20);
            SELECT TOP 1 @RndSP = MaSP FROM SAN_PHAM WHERE MaSP NOT IN (SELECT ID FROM @UsedSP) ORDER BY NEWID();
            ...
            BEGIN
                ...
                INSERT INTO CHI_TIET_MUA_HANG (MaSP, MaMuaHang, SoLuong, DonGia) VALUES (@RndSP, @MaDV, @Qty, @Price);
...
BEGIN
    ...
    BEGIN
        ...
        SELECT MaNV, @DateRun, DATEADD(MINUTE, ABS(CHECKSUM(NEWID()))%90, '07:30'), DATEADD(MINUTE, ABS(CHECKSUM(NEWID()))%90, '16:30') FROM NHAN_VIEN;
    END
    ...
    BEGIN
        ...
        SELECT MaNV, MONTH(@DateRun), YEAR(@DateRun), 160 + (ABS(CHECKSUM(NEWID()))%60), LuongCoBan, CASE WHEN (ABS(CHECKSUM(NEWID()))%10)=0 THEN 1000000 ELSE 0 END, 0, DATEADD(DAY, 5, @DateRun) FROM NHAN_VIEN;
        UPDATE BANG_LUONG_THANG SET TongLuong = LuongCoBan + Thuong WHERE Thang=MONTH(@DateRun) AND Nam=YEAR(@DateRun);
    END
...
WHILE @i <= @Total
BEGIN
    DECLARE @MaLH VARCHAR(20) = 'LH' + CAST(@i AS VARCHAR);
    DECLARE @NgayLH DATE = DATEADD(DAY, ABS(CHECKSUM(NEWID())) % DATEDIFF(DAY, @NgayStart, GETDATE()), @NgayStart);
    ...
    INSERT INTO LICH_HEN (MaLichHen, NgayHen, GioHen, TrangThai, GhiChu, MaKH, MaBS, MaCN)
