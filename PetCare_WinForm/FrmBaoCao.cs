...
namespace PetCare_WinForm
{
    public partial class FrmBaoCao : Form
    {
        ...
        private void LoadComboBoxData()
        {
            ...
            for (int i = namHienTai; i >= namHienTai - 5; i--)
                cboNam.Items.Add(i.ToString());
            cboNam.SelectedIndex = 1; // Mặc định chọn năm hiện tại để tiện xem

            // 2. Load Tháng (MỚI THÊM)
            cboThang.Items.Add("--- Tất cả ---");
            for (int i = 1; i <= 12; i++)
            {
                cboThang.Items.Add("Tháng " + i);
            }
            cboThang.SelectedIndex = 0; // Mặc định chọn tất cả tháng
        ...
        // --- HÀM LẤY THAM SỐ (ĐÃ CẬP NHẬT LOGIC THÁNG) ---
        private (object MaCN, object TuNgay, object DenNgay) GetFilterParams()
        {
            object maCN = DBNull.Value;
            object tuNgay = DBNull.Value;
            ...
            // Nếu có chọn Năm (Index > 0 vì 0 là "Tất cả")
            if (cboNam.SelectedIndex > 0)
            {
                int nam = int.Parse(cboNam.SelectedItem?.ToString() ?? DateTime.Now.Year.ToString());
                int thang = cboThang.SelectedIndex; // 0 là Tất cả, 1 là Tháng 1...
                ...
                else 
                {
                    ...
                    // Lấy ngày cuối cùng của tháng đó (xử lý năm nhuận tự động)
                    int ngayCuoiThang = DateTime.DaysInMonth(nam, thang);
                    denNgay = new DateTime(nam, thang, ngayCuoiThang);
                }
            }
